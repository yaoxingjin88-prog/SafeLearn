      ...
      <el-col :span="8" v-for="scenario in scenarios" :key="scenario.id">
        <el-card class="training-card" shadow="hover">
          <div class="card-header">
            <el-icon :size="40" :color="getDifficultyColor(scenario.difficulty)">
              <Cpu />
            </el-icon>
          ...
          <div class="card-meta">
            ...
            <span><el-icon><List /></el-icon> {{ scenario.decisionPoints.length }}个决策点</span>
          </div>
          <el-button type="primary" class="w-full mt-4" @click="$router.push(`/training/${scenario.id}`)">
            开始训练
          </el-button>
        </el-card>
      </el-col>
...
import { Cpu, Timer, List } from '@element-plus/icons-vue'
